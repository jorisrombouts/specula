...
- 2026-07-06 — **frontend-wiring MERGED (PR #1, `7e563fb`)** — M2 COMPLETE. App is fully API-backed: real bffFetch (jose HS256), dev-bypass identity, seeder→13 M1 postings, jobs lens-switch re-fetches server-filtered, auth() deduped via React cache() (nested bffFetch kept losing the session). Test harness runs the real stack (uvicorn + seeded Postgres) under Playwright + the web CI job; visual baselines regenerated against API data. **CI fully green: api + web (E2E 9/9 + visual 11/11).** Open follow-ups: (a) lens_filter scope-regex reads region label 'EU' as a country code (worked around in seed); (b) JobOut doesn't surface posting_state.note on the drawer read (persists fine); (c) local Docker `just visual-update` can't next-build offline (Google fonts) — baselines regenerate via CI.
