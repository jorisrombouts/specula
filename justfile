...
# DEV_AUTH_BYPASS is double-gated in (app)/layout.tsx (dev + this flag) so it can
# NEVER run in prod.
#
# dev-web without the Google sign-in guard — local UI work only
dev-web-noauth:
...
# enrich+crawl+extract+embed+score; prints the resulting job(s).
#
# COSTS CREDITS. Ingest one company by domain: just live-ingest acme.com
live-ingest domain:
    cd apps/api && PIPELINE_MODE=record uv run python -m specula_api.cli ingest {{domain}}

# Discover, ingest the first ATS-detected approval (one company only — cost guardrail),
# print its scored jobs.
#
# COSTS CREDITS. End-to-end proof against the real internet — see docs/RUNNING-LIVE.md
prove-live:
    cd apps/api && PIPELINE_MODE=record uv run python -m specula_api.cli prove-live

# Extensions + the specula_app grant need the superuser; migrate/seed then run as the
# non-superuser specula_app.
#
# Create + prepare a database on the shared container
db-create db:
...
# Give a worktree its own database so two branches with divergent migration heads
# don't fight over one. e.g. just db-bootstrap specula_wt_lenses
#
# Bring a per-worktree DB fully online: create + migrate + seed
db-bootstrap db:
    just db-create {{db}}
...
# Runs in the pinned Playwright image (matches CI's Chromium + font stack). The visual
# config builds and serves a production `next start` inside the container (no dev overlay,
# no on-demand compile), so baselines are deterministic. Repo is mounted; node_modules is
# masked by anonymous volumes so the host's darwin install never leaks into the Linux
# container. Bump the image tag when @playwright/test is upgraded.
#
# Regenerate the committed Linux pixel baselines
visual-update:
    docker run --rm \
...
# Runs in the pinned image WITHOUT updating baselines, reproducing CI's pixel
# comparison locally.
#
# Compare-only visual suite — see visual-update to regenerate baselines
visual:
    docker run --rm \
      -v "{{justfile_directory()}}:/work" -w /work \
      -v /work/node_modules \
